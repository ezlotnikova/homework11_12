...
public class ConnectionRepositoryImpl implements ConnectionRepository {
    ...
    public Connection getConnection() throws SQLException {
        if (ds == null) {
            ...
            config.setJdbcUrl(propertyUtil.getConfigProperty(DATABASE_URL));
            config.setUsername(propertyUtil.getConfigProperty(DATABASE_USERNAME));
            config.setPassword(propertyUtil.getConfigProperty(DATABASE_PASSWORD));
            config.addDataSourceProperty(PREPARED_STATEMENTS_CACHE_ENABLE,
                    propertyUtil.getConfigProperty(PREPARED_STATEMENTS_CACHE_ENABLE));
            config.addDataSourceProperty(PREPARED_STATEMENTS_CACHE_SIZE,
                    propertyUtil.getConfigProperty(PREPARED_STATEMENTS_CACHE_SIZE));
            config.addDataSourceProperty(PREPARED_STATEMENTS_CACHE_LIMIT,
                    propertyUtil.getConfigProperty(PREPARED_STATEMENTS_CACHE_LIMIT));
